...
interface ImageStatusDao {
    ...
    @Query("UPDATE $IMAGE_STATUS_TABLE_NAME SET status=:status, errorMessage =:errorMessage, modifiedDate =:modifiedDate,retryCount =:retryCount WHERE id=:eventId AND mobileNumber =:mobileNumber")
    ...
    @Query("UPDATE $IMAGE_STATUS_TABLE_NAME SET blobUrl =:blobUrl,errorMessage=:errorMessage where id =:imageStatusId AND mobileNumber =:mobileNumber")
    fun updateBlobUrl(
        blobUrl: String,
