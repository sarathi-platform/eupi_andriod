        ...
        try {
            ...
            if (userApiResponse.status.equals(SUCCESS, true)) {
                userApiResponse.data?.let {
                    prefRepo.savePref(PREF_KEY_USER_NAME, it.username ?: BLANK_STRING)
                    prefRepo.savePref(PREF_KEY_NAME, it.name ?: BLANK_STRING)
                    prefRepo.savePref(PREF_KEY_EMAIL, it.email ?: BLANK_STRING)
                    prefRepo.savePref(PREF_KEY_IDENTITY_NUMBER, it.identityNumber ?: BLANK_STRING)
                    prefRepo.savePref(PREF_KEY_PROFILE_IMAGE, it.profileImage ?: BLANK_STRING)
                    prefRepo.savePref(PREF_KEY_ROLE_NAME, it.roleName ?: BLANK_STRING)
                    prefRepo.savePref(PREF_KEY_TYPE_NAME, it.typeName ?: BLANK_STRING)
