...
val DEFAULT_CALCULATION_RESULT = 0F

object CalculatorUtils {

    private val tag = CalculatorUtils::class.java.simpleName
    ...
    private fun applyOperator(operatorX: Char, a: Float, b: Float): Float {
        when (operatorX) {
            ...
            '-' -> return b - a
            '*' -> return b * a
            '/' -> if (a != DEFAULT_CALCULATION_RESULT) return b / a
        }
        ...
        try {
            ...
            while (i < myChar.size) {
                if (myChar[i] in '0'..'9'
                    ...
                    while (!operatorStack.empty() && checkPrecedence(
                            myChar[i],
                            operatorStack.peek()
                        )
                    ) {
            ...
            BaselineLogger.e(
                tag = tag,
                msg = "calculate -> exception: ${ex.message}",
                ex = ex,
                stackTrace = true
            )
            return DEFAULT_CALCULATION_RESULT
