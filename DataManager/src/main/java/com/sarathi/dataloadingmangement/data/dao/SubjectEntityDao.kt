...
interface SubjectEntityDao {
    ...
    @Query("SELECT  subject_table.subjectId, subject_table.subjectName,subject_table.crpImageName as image, subject_table.dadaName, subject_table.cohortId, subject_table.cohortName, subject_table.houseNo, subject_table.villageId, subject_table.villageName, subject_table.crpImageLocalPath, subject_table.voName, subject_livelihood_mapping_table.livelihoodId, subject_livelihood_mapping_table.type as livelihoodOrder from subject_table inner join subject_livelihood_mapping_table on subject_table.subjectId = subject_livelihood_mapping_table.subjectId where subject_table.userId = :userId and subject_livelihood_mapping_table.userId = :userId and subject_livelihood_mapping_table.livelihoodId!=-1 and subject_livelihood_mapping_table.status=1")
    fun getSubjectEntityWithLivelihoodMappingUiModelList(userId: String): List<SubjectEntityWithLivelihoodMappingUiModel>

    @Query("SELECT  subject_table.subjectId, subject_table.subjectName,subject_table.crpImageName as image, subject_table.dadaName, subject_table.cohortId, subject_table.cohortName, subject_table.houseNo, subject_table.villageId, subject_table.villageName, subject_table.crpImageLocalPath, subject_table.voName, subject_livelihood_mapping_table.livelihoodId, subject_livelihood_mapping_table.type as livelihoodOrder from subject_table inner join subject_livelihood_mapping_table on subject_table.subjectId = subject_livelihood_mapping_table.subjectId where subject_table.userId = :userId and subject_livelihood_mapping_table.userId = :userId and subject_table.subjectId = :subjectId and subject_livelihood_mapping_table.livelihoodId!=-1 and subject_livelihood_mapping_table.status=1")
    fun getSubjectEntityWithLivelihoodMappingUiModelListForSubject(
        userId: String,
        subjectId: Int
    ): List<SubjectEntityWithLivelihoodMappingUiModel>

    @Query("UPDATE subject_table SET shgVerificationStatus = :shgVerificationStatus, shgVerificationDate = :shgVerificationDate, shgName = :shgName, shgMemberId = :shgMemberId where subjectId = :subjectId and userId = :userId")
