...
import com.nudge.core.DEFAULT_STATE_ID
import com.patsurvey.nudge.RetryHelper
import com.patsurvey.nudge.activities.ui.progress.domain.useCase.FetchSelectionUserUseCase
...
abstract class BaseViewModelV2(
    val fetchSelectionUserUseCase: FetchSelectionUserUseCase
) : ViewModel() {
    ...
    open fun <T> onEvent(event: T) {
        ...
        if (event is SelectionEvents.GetStateId) {
            event.result(fetchSelectionUserUseCase.getStateId())
        }

        if (event is SelectionEvents.IsUserBpc) {
            event.result(fetchSelectionUserUseCase.isUserBpc())
        }

        if (event is SelectionEvents.GetAppLanguage) {
            event.result(fetchSelectionUserUseCase.getAppLanguage())
        }
    }
    ...
    open fun onCatchError(e: Exception, api: ApiType) {
        ...
        when (e) {
            is HttpException -> {
                NudgeLogger.d(
                    "BaseViewModel",
                    "onCatchError code: ${e.response()?.code() ?: 0}, api: ${api.name}"
                )
                when (e.response()?.code() ?: 0) {
                    RESPONSE_CODE_UNAUTHORIZED -> {
    ...
    fun isUserBPC(): Boolean {
        var isUserBPC = false
        onEvent(SelectionEvents.IsUserBpc { result ->
            isUserBPC = result
        })
        return isUserBPC
    }
    ...
    fun getAppLanguage(): String {
        var appLanguage = DEFAULT_LANGUAGE_CODE
        onEvent(SelectionEvents.GetAppLanguage { result ->
            appLanguage = result
        })
        return appLanguage
    }
