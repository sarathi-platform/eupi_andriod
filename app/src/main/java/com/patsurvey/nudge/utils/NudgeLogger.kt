...
object LogWriter {
            ...
            if (isInitializing.compareAndSet(false, true) && !isQuitting && (syslogThread == null || !syslogThread!!.isAlive)) {
                ...
                syslogFile = File.createTempFile(
                    FILE_NAME_PREFIX + syslogFileNameTimeStampFormat.format(
                        Date()
                    ), FILE_NAME_SUFFIX, NudgeCore.getAppContext().getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS)
                )
    ...
    suspend fun buildSupportLogAndShare() {
        ...
        try {
            ...
            withContext(Dispatchers.Main) {
                share(context = context, logFile, arrayOf("nudge@tothenew.com"), subject, message)?.let {
                ...
                for (logFile in catalog) {
                    if (logFile.isFile) {
                        ...
                        if (logFile.name.contains(SUPPORT_LOG_FILE_NAME_PREFIX) ||
                            ...
                            if (DEBUG) NudgeLogger.d(TAG, "Deleted logfile ${logFile.name} of size ${formatShortFileSize(context, length)}")
                            removedBytes += length
                        }
                    }
                }

                NudgeLogger.d(TAG, "Logs on Disk: ${formatShortFileSize(context, bytesOfLogFiles - removedBytes)} Cleanup Removed: ${formatShortFileSize(context, removedBytes)}")
            }
        } catch (ex: Exception) {
